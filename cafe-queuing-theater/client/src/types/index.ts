...
export interface LBMWorkerConfig {
  width: number;
  height: number;
  gridSize: number;
  params: LBMParams;
...
export interface LBMWorkerInterface {
...
export interface KDEWorkerInterface {
  init(config: KDEConfig): void;
  computeHeatmap(points: { position: Point; intensity: number }[]): Promise<HeatMap | null>;
  streamingUpdate(
